...
int main(){
	...
	srand(time(NULL));
	while(true){
		if (digitalRead(0) == 1 && digitalRead(1) == 0) informacoes();
		else if (digitalRead(1) == 1 && digitalRead(0) == 0) poema(barra);
		else if (digitalRead(1) == 1 && digitalRead(1) == 1) system("lp aperture.txt");
